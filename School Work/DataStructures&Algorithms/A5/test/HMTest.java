...
public class HMTest {
    ...
    void allMethodsTest(){
        ...
        System.out.println(Arrays.toString(hm.getChains()));

        assertEquals(2, hm.getChains()[1].get("two"));
        assertEquals(1, hm.getChains()[2].get("one"));
        assertEquals(4, hm.getChains()[4].get("four"));

        HM<Integer, Integer> timeTest = new HM<>(10000);

        // these show O(1)
        for (int i = 0; i < timeTest.getChains().length; i++) {
            final long startTime = System.currentTimeMillis();
            timeTest.put(i,i);
            final long endTime = System.currentTimeMillis();
            System.out.println("Total execution of PUT time: " + (endTime - startTime));
        }

        for (int i = 0; i < 10; i++) {
            Random index = new Random(i);
            final long startTime = System.currentTimeMillis();
            timeTest.get(index.nextInt(10000));
            final long endTime = System.currentTimeMillis();
            System.out.println("Total execution of GET time: " + (endTime - startTime));
        }
    }
}