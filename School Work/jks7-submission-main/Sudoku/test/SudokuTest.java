...
class SudokuTest {
     ...
     * This test a failed puzzle that won't work and test a 3x3
     */
    @Test
    public void testFailandPass() throws FileNotFoundException {
        Scanner scan = new Scanner(new File("sudoku-board-test.txt"));
        Sudoku puzzle = Sudoku.read(scan);

        assertFalse(puzzle.solve());

        Scanner three = new Scanner(new File("sudoku-3x3.txt"));
        Sudoku puzzleThree = Sudoku.read(three);
        assertTrue(puzzleThree.solve());
    }
}
